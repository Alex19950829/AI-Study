...
> - 📄 **大模型全景主课程 PPT**：[ai_course_ppt.html](file:///Users/a58/Downloads/%E5%AD%A6%E4%B9%A0/AI-Study/AI-Study/ai_course_ppt.html) (共 27 页精美卡片、Prompt 注入攻防与企业四条安全红线全景、大模型五层立体成本优化体系与丰富交互组件)
...
- 🧮 **全交互式实操工具箱（无需后端，纯前端闭环）**：
  ...
  - **豆包 / DeepSeek 深度思考全景（`ai_course_ppt.html` 第 10~11 页）**：1:1 动态排座实测对比、`<think>` 坐席质检真实思考链纠错与草稿自愈机制。
  - **1:1 动态 Tokenizer 分词实验室（`ai_course_ppt.html` 第 24 页）**：实时体验 Trie 前缀树最长贪婪匹配、BPE 算法合并与切词 ID 生成。
  - **4 代 AI 算法演进比对（`ai_course_ppt.html` 第 25 页）**：动态切换比对 CNN 近视眼、RNN 记忆衰减、BERT 完形填空与 GPT/ChatGPT 通用跃进。
- ⌨️ **极佳的演示操纵体验**：支持键盘左右方向键 `←` / `→` 翻页、`M` 键一键唤出全局大图目录、`Esc` 退出。

---

## 📑 模块一：AI 从入门到精通 · 27 页主课程大纲 (`ai_course_ppt.html`)
...
| **10 ~ 11** | **深度思考与推理模型全景** | 同一道排座题秒答 vs 深度思考、**二手车坐席质检真实 &lt;think&gt; 思考链纠错与草稿自愈机制** |
| **12 ~ 13** | **参数拆解与多模态原理** | 模型参数量全景拆解、为什么有的 AI 看不懂图片（多模态底层原理全景） |
| **14 ~ 16** | **微调与知识库切片实操** | 微调 vs 喂资料、企业知识库切片与 TopK 实操、4 大切片模式与配置指南 |
| **17 ~ 18** | **大模型幻觉与治理攻防** | 幻觉产生的 4 大物理根因、**Temperature 陡峭化与 Top-P 杂音剪枝**、RAG 检索增强架构 |
| **19 ~ 20** | **开源生态与 LoRA 微调** | 开源真相与选型避坑、**LoRA 低秩矩阵微调数学物理拆解 (50MB改动70亿大脑)** |
| **21** | **随机性与结果控制** | **“带权掷骰子”机理**、场景分级与 4 种消除答案漂移的硬核手段 (`Temperature=0`, `Seed=42`) |
| **22** | **AI 客服变迁与自建指南** | **传统规则客服 vs 现代 LLM+RAG 智能体对比**、5 步自建落地方案与选型框架 (Dify/FastGPT) |
| **23** | **大模型限流与用量治理** | **传统 QPS 滑动窗口 vs 现代 LLM 多维限流 (RPM+TPM)**、预扣返还、3h/5h 滑动重置与 GPU 显存级调度 |
| **24** | **Token 与词表底层原理** | **分词器 (Tokenizer) 翻译官**、Trie 前缀树最长贪婪匹配、BPE 算法与中英文 Token 效率压缩率对比 |
| **25** | **4 代 AI 算法演进** | **CNN 近视眼 vs RNN 记忆衰减 vs BERT 填空局限 vs GPT/ChatGPT 通用跃进** (动态 4-Tab 模型体验) |
| **26** | **AI PM 架构选型决策** | **4 大典型认知避坑清单与 4 维架构选型决策矩阵** (Prompt vs RAG vs 微调 vs HITL) |
| **27** | **课程全景终章总结** | **27 页全景核心认知闭环与「大模型基础 30 问」在线自测跳转链接** |
