...
> - 📄 **大模型全景主课程 PPT**：[ai_course_ppt.html](file:///Users/a58/Downloads/%E5%AD%A6%E4%B9%A0/AI-Study/AI-Study/ai_course_ppt.html) (共 35 页精美卡片、大模型五层立体成本优化体系与丰富交互组件)
...
- 🧮 **全交互式实操工具箱（无需后端，纯前端闭环）**：
  ...
  - **豆包 / DeepSeek 深度思考全景（`ai_course_ppt.html` 第 11~13 页）**：1:1 动态排座实测对比、`<think>` 草稿纸机制、自发探索 3 大机制。
  - **1:1 动态显存计算器（`ai_course_ppt.html` 第 26 页）**：支持 NVIDIA 独显（RTX 4090 / 5090D）与 Apple Silicon 统一内存（75% 可用上限换算），精算 INT4 / INT8 / FP16 显存硬开销。
  - **1:1 动态 Tokenizer 分词实验室（`ai_course_ppt.html` 第 31 页）**：实时体验 Trie 前缀树最长贪婪匹配、BPE 算法合并与切词 ID 生成。
  - **1:1 动态 Token 逐字生成与概率实时更新动画（`ai_course_ppt.html` 第 32 页）**：复现 Base 模型「紫霞捧着月光宝盒」自回归连续采样推演过程。
  - **4 代 AI 算法演进比对（`ai_course_ppt.html` 第 33 页）**：动态切换比对 CNN 近视眼、RNN 记忆衰减、BERT 完形填空与 GPT/ChatGPT 通用跃进。
- ⌨️ **极佳的演示操纵体验**：支持键盘左右方向键 `←` / `→` 翻页、`M` 键一键唤出全局大图目录、`Esc` 退出。

---

## 📑 模块一：AI 从入门到精通 · 35 页主课程大纲 (`ai_course_ppt.html`)

| 页码 | 模块主题 | 核心内容 / 动态交互组件 |
| :--- | :--- | :--- |
| **01** | **AI 工程化统一本质** | Message List 核心论断：所有工程化操作本质都在精心设计 Message List |
| **02 ~ 04** | **提示词工程与物理本质** | “背景+要求+限制”万能骨架、废话费用计算器、**2D 向量激活空间中文地图与逻辑锁定矩阵** |
| **05 ~ 08** | **Prompt 进阶与安全红线** | 4 个实战杀招、JSON/MD/XML 输出格式取舍、Prompt 注入攻击与防御图鉴、企业 AI 4 条安全红线 |
| **09** | **语法结构降本与排版** | **Token 瘦身实战**：HTML 导致费用暴涨 3~4 倍，Markdown 纯文本结构实现输出层极致降本 |
| **10** | **大模型五层立体成本优化** | **从系统角度降本 70~90%**：模型路由、语法瘦身 (YAML/CSV)、语义压缩 (Few-Shot/LLMLingua)、输出约束与 KV 缓存 |
| **11 ~ 13** | **深度思考与推理模型全景** | 同一道排座题秒答 vs 深度思考、`<think>` 草稿纸物理机制、模型自发探索 3 大核心机制 |
| **14 ~ 17** | **参数拆解与多模态原理** | 模型参数量全景拆解、小模型反杀选型、静态参数 vs 动态输入、多模态视觉原理 |
| **18 ~ 20** | **微调与知识库切片实操** | 微调 vs 喂资料、企业知识库切片与 TopK 实操、4 大切片模式与配置指南 |
| **21 ~ 22** | **大模型幻觉与治理攻防** | 幻觉产生的 4 大物理根因、**Temperature 陡峭化与 Top-P 杂音剪枝**、RAG 检索增强架构 |
| **23 ~ 25** | **开源生态与 LoRA 微调** | 开源真相与选型避坑、开源 7B 模型权重实操、**LoRA 低秩矩阵微调数学物理拆解** |
| **26** | **本地显存与硬件选型** | **1:1 动态显存计算器**、NVIDIA 独显（拼速度）vs Mac 统一内存（拼容量）全景对比 |
| **27** | **随机性与结果控制** | **“带权掷骰子”机理**、场景分级与 4 种消除答案漂移的硬核手段 (`Temperature=0`, `Seed=42`) |
| **28** | **AI 客服变迁与自建指南** | **传统规则客服 vs 现代 LLM+RAG 智能体对比**、5 步自建落地方案与选型框架 (Dify/FastGPT) |
| **29** | **大模型限流与用量治理** | **传统 QPS 滑动窗口 vs 现代 LLM 多维限流 (RPM+TPM)**、预扣返还、3h/5h 滑动重置与 GPU 显存级调度 |
| **30** | **训练 vs 推理物理全景** | **对话非学习/参数只读冻结**、4 步物理训练流（载入二进制 ➔ GPU 梯度算 Loss ➔ 导出 safetensors 文件） |
| **31** | **Token 与词表底层原理** | **分词器 (Tokenizer) 翻译官**、Trie 前缀树最长贪婪匹配、BPE 算法与中英文 Token 效率压缩率对比 |
| **32** | **Base 模型自回归预测** | **1:1 动态 Token 逐字生成与概率实时更新动画**、从上下文推演 (`「紫霞捧着月光宝盒」` ➔ `「，你说你喜欢我」`) |
| **33** | **4 代 AI 算法演进** | **CNN 近视眼 vs RNN 记忆衰减 vs BERT 填空局限 vs GPT/ChatGPT 通用跃进** (动态 4-Tab 模型体验) |
| **34** | **AI PM 架构选型决策** | **4 大典型认知避坑清单与 4 维架构选型决策矩阵** (Prompt vs RAG vs 微调 vs HITL) |
| **35** | **课程全景终章总结** | **35 页全景核心认知闭环与「大模型基础 30 问」在线自测跳转链接** |
